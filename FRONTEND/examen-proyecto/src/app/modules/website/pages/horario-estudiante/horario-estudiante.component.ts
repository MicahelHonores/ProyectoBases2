...
export class HorarioEstudianteComponent implements OnInit, AfterViewInit {
  horarios: any[] = [];
  semestres: any[] = [];
  ...
  fetchHorarios(semestre: string): void {
    ...
    this.horariosService.getHorariosEstudiante(this.semanaActual, semestre).subscribe(
      (data: any[]) => {
        this.horarios = data;
  ...
  fetchHorariosSemana(semana: number): void {
    ...
    this.horariosService.getHorariosEstudiante(semana, this.semestreActual).subscribe(
      (data: any[]) => {
        this.horarios = data;
        this.organizarHorariosPorDia();
      },
